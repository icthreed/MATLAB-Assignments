...
function x_rotated = rotation2D(x,theta)
    xi=[x(1), x(2)*i];
    theta=theta * pi/180;
    x_rotated0=xi*(cos(theta)+i*sin(theta));
    x_rotated0=struct('real', real(x_rotated0), 'imaginary', imag(x_rotated0));
    x_rotated(1)=round(sum(x_rotated0.real),2);
    x_rotated(2)=round(sum(x_rotated0.imaginary),2);
end
...
function patientStruct = combinePatient(checkIn,weighIn,triage,ECG)
    [n m] = size(checkIn);
    for i=1:n
        patientStruct(i).Name = checkIn(i,1);
        patientStruct(i).Age = str2num(checkIn(i,2));
        patientStruct(i).Sex = checkIn(i,3);
        for j=1:n
            if weighIn(j,1) == patientStruct(i).Name
                patientStruct(i).Weight = str2num(weighIn(j,2));
            end
            if triage(j,1) == patientStruct(i).Name
                patientStruct(i).HR = str2num(triage(j,2));
                systolic = triage(j,3);
                diastolic = triage(j,4);
                bp = append(systolic,"/",diastolic);
                patientStruct(i).BP = bp;
            end 
            patientStruct(i).ECG = cell2mat(ECG(i));
        end
    end
    for i=1:n
        disp(patientStruct(i));
        if patientStruct(i).ECG ~= 0
            plot(cell2mat(ECG(i)));
        end
    end
    weightg=[patientStruct.Weight]./2.205;
    for jj=1:4
        patientStruct(jj).Weight=weightg(jj);
    end
end
